...
type EngagementHint = "normal" | "low_effort" | "off_topic";
...
interface SendMessageResult {
  ...
  aiMessage: string;     // full text (kept for backward compat)
  ...
  suggestedWords?: string[]; // words to suggest adding to the user's list on completion
...
function classifyEngagement(message: string): EngagementHint {
  ...
  for (const pattern of offTopicPatterns) {
    if (pattern.test(trimmed)) return "off_topic";
  }
...
export async function sendMessage(
  ...
  if (!recentMsgs.empty) {
    ...
    const lastTimestamp = lastMsg.timestamp?.toDate?.() ?? new Date(0);
  ...
  if (newMessageCount >= SESSION_LIMITS.softWindDownAt) {
    additionalInstructions += buildWindDownInstruction();
  }
  ...
  const parallelOps: [Promise<Record<string, number>>, Promise<Array<{ role: "user" | "assistant"; content: string }>>, ...Promise<unknown>[]] = [
    scoreWordUsage(userMessage, targetWords),
  ...
  userMsgRef.update({ wordUsage: detectedWords, wordUsageScores }).catch((e) =>
    console.warn("Non-critical: failed to update word scores on message:", e)
  );
  ...
  if (shouldComplete) {
    ...
    const [bonus, suggestions] = await Promise.all([
      checkAndAwardBonusChat(db, session.userId, updatedWordBag),
      suggestRelatedWords(targetWords).catch(() => [] as string[]),
    ]);
      ...
      try {
        ...
        if (wordListId) {
          ...
          const existingWords = new Set(
            (wlDoc.data()?.words || []).map((w: string) => w.toLowerCase())
          );
