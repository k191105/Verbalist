...
class _PaywallState extends State<Paywall> {
  ...
  late final VoidCallback _entitlementsListener;

  String _getProFeatureDisplayName(
    String key, [
    String fallback = 'Premium Feature',
  ]) {
    return EntitlementsService.proFeatureDisplayNames[key] ?? fallback;
  }

  Future<void> _handleEntitlementsChanged() async {
  ...
  void initState() {
    ...
    _entitlementsListener = () {
      // Fire and forget
      // ignore: discarded_futures
      _handleEntitlementsChanged();
    };
    EntitlementsService.instance.addListener(_entitlementsListener);

    Future.microtask(() async {
      // Preload offerings and attempt a quick entitlement sync with a short timeout
      await PurchaseService.instance.ensureOfferingsLoaded();
      if (!mounted) return;

      try {
        await PurchaseService.instance.syncEntitlementFromRC().timeout(
          const Duration(seconds: 2),
        );
      } catch (_) {
        // Ignore timeout to keep the sheet snappy
      }
      if (!mounted) return;

      // If Pro already active, close immediately
      final isPro = await EntitlementsService.instance.isPro();
      if (!mounted) return;
  ...
  @override
  void dispose() {
    EntitlementsService.instance.removeListener(_entitlementsListener);
    super.dispose();
  }
  ...
  Map<String, List<String>> get _contextualBullets {
    return {
      ...
      'browse_author': [
        _getProFeatureDisplayName(EntitlementsService.browseAuthor),
        'Deep author collections',
        'Full library access (~5,000 quotes)',
        'Discover new voices',
      ],
      'browse_period': [
        _getProFeatureDisplayName(EntitlementsService.browsePeriod),
        'Unlock the premium library',
        'See era‑specific highlights',
        'Time‑based discovery made easy',
      ],
      'settings_theme': [
        _getProFeatureDisplayName(EntitlementsService.premiumThemes),
        'Share without watermark',
        'Home/lock‑screen widgets',
        'Make it feel uniquely yours',
      ],
      'settings_font': [
        _getProFeatureDisplayName(EntitlementsService.premiumFonts),
        'Beautiful, readable typography',
        'Share without watermark',
        'Make every quote look right',
      ],
      'profile_upgrade': EntitlementsService.proFeatureDisplayNames.values
          .toList(),
      'reward_upgrade': [
        'Keep your unlocked feature forever',
        ...EntitlementsService.proFeatureDisplayNames.values,
      ],
      'learn_trainer': [
        _getProFeatureDisplayName(EntitlementsService.learnTrainer),
        'Adaptive difficulty',
        'Review mistakes faster',
        'Master quotes with spaced repetition',
      ],
      'srs_unlimited': [
        _getProFeatureDisplayName(EntitlementsService.srsUnlimited),
        'Master quotes with spaced repetition',
        'Track your progress',
  ...
  Widget _buildAllProFeaturesExpandable() {
    final Map<String, List<String>> groups = {
      ...
      'Customize': [
        _getProFeatureDisplayName(EntitlementsService.premiumThemes),
        _getProFeatureDisplayName(EntitlementsService.premiumFonts),
        _getProFeatureDisplayName(EntitlementsService.premiumShareStyles),
      ],
      'Learn': [
        _getProFeatureDisplayName(EntitlementsService.srsUnlimited),
        _getProFeatureDisplayName(EntitlementsService.learnTrainer),
