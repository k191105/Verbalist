 ...
 */
function regexPreCheck(message: string, targetWords: string[]): string[] {
 ...
 */
export async function scoreWordUsage(
  userMessage: string,
  targetWords: string[]
...
If no target words were used, respond with: {}`;
  ...
  const client = getClient();
  const response = await client.chat.completions.create({
    model: LLM_MODELS.wordUsageScoring,
...
Respond ONLY with a JSON array of lowercase words. No explanation.`;
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.filter((w): w is string => typeof w === "string").slice(0, 8);
    }
    return [];
  } catch {
